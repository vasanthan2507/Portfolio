...
export default function InputField({ label, type = 'text', name, value, onChange, error, required, autoComplete }) {
  const [focused, setFocused] = useState(false);
  const active = focused || value.length > 0;

  return (
    <div style={{ position: 'relative' }}>
      {/* Honeypot fields must NOT use this component — handled separately */}
      <input
        type={type}
        id={name}
        name={name}
